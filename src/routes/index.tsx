...
function AcademicPortfolio() {
  ...
  const t = (key: string, fallback: string) => data.content[key]?.trim() || fallback;

  const siteName = t("site_name", "Harun Chaudhari");
  const cvUrl = t("cv_url", "");
  const linkedinUrl = t("linkedin_url", "https://www.linkedin.com");
  const scholarUrl = t("scholar_url", "https://scholar.google.com");
  ...
  const handleCv = () => {
    if (cvUrl) window.open(cvUrl, "_blank", "noopener");
    else downloadPlaceholder("Harun-Chaudhari-CV.txt", `Curriculum Vitae — ${siteName}`);
  };
    ...
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-primary-foreground/10 bg-primary/95 text-primary-foreground backdrop-blur-md">
        <div className="mx-auto flex h-18 max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-12">
          <a href="#home" className="group flex items-center gap-3" aria-label={`${siteName}, home`}>
            <span className="grid size-9 place-items-center border border-primary-foreground/30 font-serif text-sm font-bold">HC</span>
            <span className="hidden font-serif text-base sm:block">{siteName}</span>
          </a>
          <nav className="hidden items-center gap-7 lg:flex" aria-label="Main navigation">
            ...
            <Link to="/auth" className="flex items-center gap-1.5 border border-primary-foreground/25 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-primary-foreground/75 transition-colors hover:border-accent hover:text-accent">
        ...
        {menuOpen && (
          <nav className="border-t border-primary-foreground/10 bg-primary px-5 py-4 lg:hidden" aria-label="Mobile navigation">
            {navigation.map(([label, id]) => (
              <a key={id} href={`#${id}`} onClick={() => setMenuOpen(false)} className="block border-b border-primary-foreground/10 py-3 text-sm text-primary-foreground/80">
                {label}
              </a>
            ))}
            <Link to="/auth" onClick={() => setMenuOpen(false)} className="flex items-center gap-2 py-3 text-sm text-accent">
          ...
          <div className="relative mx-auto w-full max-w-7xl px-5 pb-16 sm:px-8 sm:pb-20 lg:px-12 lg:pb-24">
            <div className="max-w-3xl">
              <div className="mb-7 flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.22em] text-accent">
                <span className="h-px w-10 bg-accent" /> {t("hero_eyebrow", "English Literature & Academic Research")}
              </div>
              <h1 className="font-serif text-5xl leading-[1.08] sm:text-6xl lg:text-7xl">{siteName}</h1>
              <p className="mt-6 max-w-2xl text-lg leading-relaxed text-primary-foreground/80 sm:text-xl">{t("hero_subtitle", "Ph.D. Candidate in English Literature & Academic Researcher")}</p>
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Button variant="hero" size="lg" asChild><a href="#research">View Research <ArrowDown /></a></Button>
                <Button variant="heroOutline" size="lg" onClick={handleCv}><Download /> Download Full CV</Button>
              </div>
            </div>
        ...
        <section id="about" className="scroll-mt-18 bg-background py-20 sm:py-28">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20 lg:px-12">
            <div>
              ...
              <p className="mt-7 max-w-xl whitespace-pre-line text-base leading-8 text-muted-foreground">{t("about_body", "A dedicated literature scholar with a focused interest in seventeenth-century prose.")}</p>
              <blockquote className="mt-8 border-l-2 border-accent pl-5 font-serif text-lg italic leading-relaxed text-primary">{t("about_quote", "“To read deeply is to enter a conversation across centuries.”")}</blockquote>
            </div>
            <div className="border border-border bg-card shadow-sm">
              ...
              <dl className="divide-y divide-border">
                {data.bioRows.map((row) => (
                  <BioRow key={row.id} term={row.term} value={row.value} important={row.important} />
                ))}
              </dl>
            </div>
          </div>
          ...
          <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
            ...
            <div className="mt-10 grid overflow-hidden border border-border bg-card shadow-md lg:grid-cols-[0.37fr_0.63fr]">
              <div className="flex min-h-72 flex-col justify-between bg-primary p-8 text-primary-foreground sm:p-10">
                ...
                <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent">Current doctoral research</p><p className="mt-3 whitespace-pre-line text-sm leading-6 text-primary-foreground/60">{t("research_field", "English Literature\nSeventeenth-Century Studies")}</p></div>
              </div>
              <div className="p-8 sm:p-10 lg:p-14">
                <Quote className="size-8 text-accent" />
                <h2 className="mt-6 font-serif text-3xl leading-tight sm:text-4xl">{t("research_title", "“Logic and mysticism in the selected works of Thomas Browne”")}</h2>
                <p className="mt-7 whitespace-pre-line leading-8 text-muted-foreground">{t("research_body", "")}</p>
          ...
          <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
            ...
            <div className="mt-10 grid gap-12 lg:grid-cols-2 lg:gap-20">
              <div className="relative border-l border-border pl-8">
                ...
                <h2 className="mt-3 font-serif text-2xl leading-snug sm:text-3xl">{t("teaching_role", "Assistant Professor of English")}</h2>
                <p className="mt-3 text-base font-semibold text-primary">{t("teaching_institution", "Shree Morarji Desai Arts & Com. College, Buhari")}</p>
                <p className="mt-6 whitespace-pre-line leading-8 text-muted-foreground">{t("teaching_body", "")}</p>
              </div>
              <div>
                ...
                <p className="mt-5 whitespace-pre-line leading-8 text-muted-foreground">{t("pedagogy_body", "")}</p>
                <div className="mt-7 flex flex-wrap gap-2">{splitTags(t("pedagogy_tags", "Environmental Literature, Literary Theory, English Grammar")).map((tag) => <Tag key={tag}>{tag}</Tag>)}</div>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20 lg:px-12">
            ...
            <form onSubmit={handleSubmit} className="grid gap-5" aria-label="Contact form">
              ...
              <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
                <Button variant="hero" size="lg" type="submit" disabled={sending}><Send /> {sending ? "Sending..." : "Submit Message"}</Button>
                {sent && <p className="flex items-center gap-2 text-sm text-accent" role="status"><CheckCircle2 className="size-4" /> Thank you. Your message has been received.</p>}
      ...
      <footer className="bg-footer text-footer-foreground">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-5 py-8 sm:px-8 md:flex-row md:items-center md:justify-between lg:px-12">
          <div><p className="font-serif">{siteName}</p><p className="mt-1 text-xs text-footer-foreground/55">© 2026. Academic Portfolio. All rights reserved.</p></div>
          <div className="flex items-center gap-2">
            <a href={linkedinUrl} target="_blank" rel="noreferrer" className="grid size-10 place-items-center border border-footer-foreground/20 transition-colors hover:border-accent hover:text-accent" aria-label="LinkedIn"><Linkedin className="size-4" /></a>
            <a href={scholarUrl} target="_blank" rel="noreferrer" className="grid size-10 place-items-center border border-footer-foreground/20 transition-colors hover:border-accent hover:text-accent" aria-label="Google Scholar"><BookMarked className="size-4" /></a>
            <Link to="/auth" className="ml-2 text-xs uppercase tracking-[0.14em] text-footer-foreground/55 transition-colors hover:text-accent">Admin</Link>
          </div>
        </div>
      </footer>
    </div>
